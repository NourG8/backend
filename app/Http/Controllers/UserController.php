...
class UserController extends Controller
{
    public function getAllUsers()
    {
        ...
        $users = User::where([['users.status', '=', "active"],['users.is_deleted','=',0]])->with([
            'positions'=> fn($query) => $query->where('endDate', '=', null)->with('user'),
            'teamUser'=>[
                'team'=>[
                    'department'
                ]
        ...
        foreach ($users as $user) {
            $List_team_id= [];
            foreach ($user['teamUser'] as $team) {
                if($team['team']['id']){
                    array_push($List_team_id,$team['team']['id']);
                }
                $user['List_team_id'] = $List_team_id;
                $user['department_id'] = $team['team']['department_id'];
            }
            foreach ($user['positions'] as $pos) {
                $user['position_id'] = $pos['position_id'];
            }
        }
        ...
        //     ->select('users.*','position_users.position_id','teams.department_id', 'team_users.team_id')
        //     ->where([
        //         ['users.status', '=', "active"],
        //         ['users.is_deleted','=',0],
    ...
    public function getArchivedUser()
    {
          $users = DB::table('users')
            ->leftJoin('position_users', 'position_users.user_id', '=', 'users.id')
            ->select('users.*','position_users.position_id')
            ->where([
    ...
    public function AddUser(Request $request)
    {
        ...
        $user->emailProf = $request->input('emailProf');
        $user->address = $request->input('address');
        $user->image = "img.png";
        $user->dateBirth = $request->input('dateBirth');
        $user->placeBirth =$request->input('placeBirth');
        $user->nationality = $request->input('nationality');
        $user->phone = $request->input('phone');
        $user->phoneEmergency = $request->input('phoneEmergency');
        $user->FamilySituation = $request->input('FamilySituation');
        $user->nbChildren = $request->input('nbChildren');
        $user->levelStudies = $request->input('levelStudies');
        $user->specialty = $request->input('specialty');
        $user->matricule = $request->input('matricule');
        $user->carteId = $request->input('carteId');
        $user->status = "active";
        $user->regimeSocial =  $request->input('regimeSocial');
        $user->text = $request->input('text');
        $user->pwd_reset_admin = 0;
        $user->cin = $request->input('cin');
        $user->deliveryDateCin = $request->input('deliveryDateCin');
        $user->deliveryPlaceCin = $request->input('deliveryPlaceCin');
        $user->numPassport = $request->input('numPassport');
        $user->motivation = $request->input('motivation');
        $user->integrationDate = $request->input('integrationDate');
        ...
        $position_user->position_id = $request->input('position_id');
        $position_user->startDate = now();
        $position_user->user_id = $user->id;
        $position_user->save();

        $teams_user = new TeamUser();
        $teams_user->team_id = $request->input('team_id');
        $teams_user->is_leader =0;
        $teams_user->integration_date =now();
    ...
    {
        $user_contract = UserContract::findOrFail($id);
        $user_contract->is_deleted = 1;
        $user_contract->save();
